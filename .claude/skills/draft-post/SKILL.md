...
4. If Opportunity proof is missing or the selected draft does not materially use it, report the failed proof gate. Never invent or infer ownership. The user may rerun with `--proof-type` and `--proof-value`.
