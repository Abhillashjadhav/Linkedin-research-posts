...
- set `proof_id` to `NOT_REQUIRED`; author proof is not an input requirement;
...
- the supplied public evidence supports the judgment;
...
4. **Proposed intervention:** an evidence-grounded decision practice that could reduce the risk; describe it as a recommendation, never as an unverified personal achievement.
