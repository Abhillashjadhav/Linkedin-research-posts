...
7. invoke the separate `Abhillashjadhav/no-ai-slop` `SKILL.md` plus `eval.md`;
8. when configured, invoke PM Human Writer with an owner-only profile beneath `data/private/`, then re-run Critic and deterministic gates after either editor changes the post;
9. write and score the first comment, then run deterministic evidence checks, integrated anti-slop, and the same separate artisanal edit;
10. invoke Artifact Editor only after text is locked, render the plan as SVG, and invoke Visual QA with deterministic layout metadata;
11. persist machine-readable and human-readable traces for executed days ending
...
reasoning. Publishing remains disabled and human approval remains unrecorded. The private
voice profile is treated as untrusted style guidance, never as evidence or task
instructions. Its contents and path are excluded from persisted traces.
...
The `published_posts` row snapshots the package creation time, chosen candidate, goal/format/slot, revision metadata, five Critic axes, raw/effective total, hook-cap state, rank, whether it was recommended, and a SHA-256 fingerprint of the exact descriptor-held brief/candidate snapshot. It stores no candidate body or private package content. The fingerprint is immutable from the first recording. Every pre-v4 migration forces existing fingerprints to `NULL`, including an interrupted migration where the column already exists; those rows remain metric-eligible but always have a learning-context gap. A database already claiming v4 with the column missing fails for explicit recovery instead of self-repairing. The asserted publication time cannot precede package creation. `performance_observations` then keys each snapshot by package, checkpoint, and `organic|paid` channel. Organic and paid data never overwrite or aggregate into one another.
...
An exact repeat is idempotent. A conflicting row requires `--replace`, a complete metric snapshot, an equal-or-newer observation timestamp, and a correction recording time no earlier than the stored `updated_at`; a clock rollback therefore cannot backdate learning state. CSV imports use the exact header order documented in the README, live package validation for every row, duplicate-key rejection, and one database transaction. `init` secures the fixed private input directory and ledger; nested CSV directories must be mode `0700` and CSV files mode `0600`. Package directories and all six committed files must retain the same owner-only invariants produced by package generation. Older free-form performance tables from the recovered draft implementation are renamed to `legacy_performance_unverified` and preserved, but they are not treated as learning evidence because they lack trusted package/candidate provenance.

### Weekly learning loop

`weekly-review` is cumulative and observation-only. An explicit `--as-of` value must be a timezone-aware whole-second timestamp; rows whose observation or last update is later than that boundary are excluded because the runtime cannot reconstruct the earlier value of a corrected snapshot. The generated JSON is written atomically as an owner-only file beneath ignored `data/private/weekly-reviews/`. Repeating the same review is idempotent. Stdout contains only the private report path plus counts and status, never candidate text, evidence, URLs, or proof fields.

Canonical learning evidence is exactly one package-linked organic `72h` row per published package observed from 72 hours inclusive to 96 hours exclusive after publication. The storage checkpoint accepts a broader 72-hour-to-seven-day collection window, but later rows are counted as exposure-incomparable exclusions rather than allowed to beat a 73-hour post with cumulative time. Earlier `2h`/`24h` rows are immature. Organic `7d` data is a separate follow-up cohort rather than a replacement or blend, and every paid row is explicitly descriptive-only. No channel or checkpoint fallback exists. Legacy free-form rows never enter the joined performance query.

Actual outcomes use goal-specific lexicographic vectors rather than hidden weights:

- Reach: `(non_follower_reach, impressions)`.
- Authority: `(saves + sends + reposts, external_comments, profile_visits)`.
- Opportunity: `(qualified_inbound, github_clicks, relevant_followers, profile_visits)`, where qualified inbound is the sum of the three inbound metrics.

The review returns all tied leading package/candidate references for each goal, their stored `hook_strength`, and the observed outcome vector. Winner selection applies the channel, checkpoint, exposure-age, and `--as-of` filters before any package file is opened; nonleaders, late rows, future corrections, and unanchored legacy rows are never opened. For anchored leaders only, it reopens the fixed six-file private package as one descriptor-held snapshot, repeats the complete live eligibility validation, verifies the stored fingerprint, and strictly parses the committed candidate and brief Markdown. The planned route must be the exact canonical route for the stored goal. It exposes a capped first-paragraph hook excerpt, candidate angle, route, and paragraph count. The full body, sources, proof fields, and private paths remain excluded. Missing, removed, changed, raced, malformed, or unanchored package context becomes an explicit gap. These are observed winning-context references, not proof that wording or structure caused the result. The original three-candidate ranking is still not testable against actual outcomes because only the candidate manually published outside the runtime has observations.

Cross-package Critic alignment is computed only within one goal. Pairs tied on actual outcome or Critic effective/raw totals are not scorable. A verdict remains `INSUFFICIENT` until at least three distinct posts produce at least three scorable pairs; only then can concordant and discordant counts yield `MATCHED`, `DID_NOT_MATCH`, or `MIXED`.

Rubric calibration is never automatic. For one goal, known output format, and Critic axis, the score-5 and score-4-or-lower cohorts must each have at least three distinct comparable organic-72h posts and share at least two ISO publication weeks. A suggestion requires both a strictly greater pooled lower-score median and independent lower-score outperformance in every shared week. Threshold shortfalls remain evidence gaps; a fully evaluated non-reversal becomes an explicit `ASSESSED_NO_REPEATED_REVERSAL` with no rubric change. Only repeated evidence emits `REVIEW_AXIS_CALIBRATION`. The command does not edit the recovered Critic rubric, prompts, voice anchors, packages, database, approval state, or publishing state.

### Production hardening

`init` is the explicit state-creation and schema-migration command. Both mutating database access and `doctor` anchor the production ledger at the repository descriptor, open and revalidate every private-path component with no-follow semantics, and create missing private parents only descriptor-relative; an intermediate symlink cannot redirect setup or recording. Read-only inspection accepts only the four exact canonical SQLite autoindexes alongside the allowed tables. Any other internal or external object, non-table application object, or duplicate name fails schema attestation. Database sidecars are checked both before and after the immutable read so newly appearing WAL state cannot be ignored. The weekly-report writer applies the same component-wise descriptor boundary before creating its final private directory or file. Diagnostics do not create a missing ledger, upgrade an old schema, change modes, or inspect environment values. Secure POSIX descriptor, ownership, and mode primitives are a runtime requirement; unsupported platforms fail closed.

`privacy-check` enumerates Git-tracked and prospective non-ignored worktree files and separately reads each exact stage-0 regular blob from the Git index. It refuses private/generated paths, databases and sidecars, high-confidence secret or SQLite signatures, scheduled Actions workflows, and runtime LinkedIn/browser write surfaces. Reads are bounded and compared across independent positional and streamed snapshots; every worktree directory edge, leaf, and repository-root identity is held and revalidated without following links, while a content mutation hidden by coarse timestamps or an index change during scanning fails closed. Ignored private data is not traversed and matching values are never echoed. `make check` runs that gate before the warnings-as-errors suite, and the push/pull-request workflow has read-only contents permission, finite timeouts, immutable action pins, and no scheduled trigger.

## Safety boundary

Source bodies, candidates, public proof fields, package files, CSV rows, metrics, and generated learning summaries are untrusted data, not instructions. Analysis, routing, all five gates, eligibility, package rendering, performance validation, and weekly comparison are deterministic Python. Only an explicitly consented live drafting run can cross the Writer or Critic model boundary, under the zero-tools and no-session restrictions above. Critic scores, gate passes, package recommendations, performance records, alignment results, and calibration suggestions are not human approval. There is no browser, Gmail, LinkedIn write, approval-recording command, scheduler, rubric auto-tuner, or automatic publishing action.
