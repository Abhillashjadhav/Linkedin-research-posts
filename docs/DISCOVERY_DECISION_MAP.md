...
| Evidence retrieval | `daily_spine_cli.resolve_signal_evidence`, `_resolve_parallel_evidence` | Reuse matching verified evidence, then use up to three workers on disjoint ranked lead batches under one shared 180-second deadline. Keep valid results from completed workers and expose timeout warnings; all-timeout remains resumable. Require actual bodies, exact lead bindings, valid URLs and dates; deduplicate to one to seven verified sources. |
...
| Frozen weekday purpose | `weekly_spine`, `config/weekly-spine.json` | Posting date selects Monday resources, optional Tuesday incident + mitigation, Wednesday own build + supplied real video, Thursday incident + mitigation, Friday short sarcastic AI humour. Narrative devices and inventory reuse must remain within that purpose. |
| Writing acceptance | `acceptance_policy`, `quality_optimizer`, `eval_package` | All live single-post styles: one scored batch; shortlist only totals >18 and rank hook first, then total. Other findings are advisory. All evaluated text is retained beside its scores, including below-bar results. |
