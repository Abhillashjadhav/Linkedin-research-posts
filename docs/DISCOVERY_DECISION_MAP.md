...
| Writing acceptance | `acceptance_policy`, `quality_optimizer`, `eval_package` | Standard: hook and voice >=4; other axes >=3 (minimum total 17), bounded repair. `--post-style short-humorous`: one scored batch; shortlist only totals >18 and rank hook first, then total. Other findings are advisory. All evaluated text is retained beside its scores, including below-bar results. |
