...
class CriticScoreValidationTests(unittest.TestCase):
    ...
    def test_live_hook_and_voice_anchors_match_the_locked_contract(self) -> None:
        ...
        self.assertIn("not as an imitation of the owner", role)
        self.assertIn("stacked punchy fragments", role)
        self.assertIn("Yes, you read that right", role)
        self.assertIn("It didn't misunderstand the rule", role)
        self.assertIn("optional devices, never a checklist", role)
        ...
        self.assertIn("not as an imitation of the owner", voice["4"])
        self.assertIn("directly publishable", voice["4"])
        self.assertIn("without performing wit", voice["5"])
        self.assertIn("presentation language", voice["banned_register"]["rule"])
        self.assertIn("AI scaffolding", voice["3"])
