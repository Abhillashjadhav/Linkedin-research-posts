...
class RegisterTests(unittest.TestCase):
    ...
    def test_his_plain_and_domain_vocabulary_is_never_flagged(self):
        for line in ("A friend at a frontier AI lab making just short of a million dollars a year.",
                     ...
                     "A dependable system inherits checks that were not examined.",
                     "Most teams inspect the evidence before launch."):
    ...
    def test_named_off_register_idiom_fails_deterministically(self):
        v = hs.evaluate("A friend at a frontier AI lab making just shy of a million dollars.")
        self.assertEqual(v.status, "FAIL")
        self.assertEqual(v.reason_code, "off-register-word")
        self.assertIn("shy of", v.evidence)

    def test_capitalised_product_name_is_not_flagged_as_register(self):
        self.assertEqual(hs.off_register_words("AcmeQuasar keeps the agent safe."), [])
