...
This script is executed against a detached comparison worktree by the current runner.
It never changes release behavior: it only observes the existing quality-loop callbacks
and writes their already-computed candidates, scores, gates, and feedback to one private
JSON file outside the worktree.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
        ...
        cycles.append(
            {
                ...
                "accepted_candidate_ids": [
                    str(getattr(item, "candidate_id", "")) for item in accepted
                ],
    ...
    def capture_success(
        attempt: object, accepted: Sequence[object], cycle: int, limit: int
    ) -> None:
        record_attempt(
            attempt,
            cycle=cycle,
