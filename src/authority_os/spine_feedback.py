...
def validate_record(record: Mapping[str, object]) -> dict[str, object]:
    ...
    published_at, published_local = _timestamp(record["published_at"], field="published_at")
    ...
    if record["weekday"] != weekday:
        raise workflow.WorkflowError("weekday must match the supplied publication timestamp.")
    ...
    if selected_spine not in CONTENT_SPINES:
...
def _private_root(root: Path) -> int:
    ...
    try:
        ...
        raise workflow.WorkflowError("Private feedback directory is unavailable or unsafe.") from exc
    if (
        ...
        os.close(descriptor)
        raise workflow.WorkflowError("Private feedback directory must be owner-only (0700).")
    return descriptor
...
def _feedback_location(
    ...
    *,
    ...
    if candidate.parent != root or candidate.name in {"", ".", ".."}:
        raise workflow.WorkflowError("Spine feedback must remain directly under data/private.")
...
def append_record(
    ...
    *,
    ...
    root, filename = _feedback_location(
        path, private_root=private_root, allow_test_root=_allow_test_root
    )
    ...
    try:
        ...
        if (
            ...
            raise workflow.WorkflowError("Private spine feedback file is unavailable or unsafe.")
        payload = (json.dumps(validated, sort_keys=True, separators=(",", ":")) + "\n").encode()
        if metadata.st_size + len(payload) > MAX_FILE_BYTES:
            raise workflow.WorkflowError("Private spine feedback file reached its size limit.")
        written = os.write(descriptor, payload)
        if written != len(payload):
            raise workflow.WorkflowError("Spine performance record was not written completely.")
        os.fsync(descriptor)
    except OSError as exc:
        raise workflow.WorkflowError("Private spine feedback file is unavailable or unsafe.") from exc
    finally:
        if descriptor >= 0:
            os.close(descriptor)
        os.close(root_fd)
    ...
    *,
    ...
    try:
        ...
        while remaining:
            ...
            if not chunk:
                raise workflow.WorkflowError("Private spine feedback file changed while reading.")
            chunks.append(chunk)
            remaining -= len(chunk)
        raw = b"".join(chunks).decode("utf-8")
    except UnicodeDecodeError as exc:
        raise workflow.WorkflowError("Private spine feedback file is not valid UTF-8.") from exc
    except OSError as exc:
        raise workflow.WorkflowError("Private spine feedback file is unavailable or unsafe.") from exc
    finally:
        if descriptor >= 0:
    ...
    for line in lines:
        try:
            ...
            raise workflow.WorkflowError("Private spine feedback contains invalid JSONL.") from exc
        if not isinstance(parsed, Mapping):
            raise workflow.WorkflowError("Private spine feedback contains an invalid record.")
        records.append(validate_record(parsed))
    return records


def _latest_records(records: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    ...
    for raw in records:
        ...
        if previous is None or (
            str(record["observed_at"]), str(record["recorded_at"])
        ) > (
            str(previous["observed_at"]), str(previous["recorded_at"])
        ):
            latest[key] = record
...
def summarise(
    records: Sequence[Mapping[str, object]], *, include_outliers: bool = False
) -> dict[str, object]:
    ...
    baseline = latest if include_outliers else [
        record for record in latest if not bool(record["is_breakout_outlier"])
    ]
    by_spine: dict[str, dict[str, object]] = {}
    for spine in CONTENT_SPINES:
        cohort = [record for record in baseline if record["selected_spine"] == spine]
        rates = [
        ...
        by_spine[spine] = {
            ...
            "median_impressions": _median([int(record["impressions"]) for record in cohort]),
            "median_engagements": _median([int(record["engagements"]) for record in cohort]),
            "median_engagement_rate_pct": _median(rates),
            "sample_status": (
                "READY_TO_COMPARE" if len(cohort) >= MIN_COMPARABLE_SAMPLE else "INSUFFICIENT_SAMPLE"
            ),
        }
    return {
...
def parser() -> argparse.ArgumentParser:
    ...
    record = subparsers.add_parser("record", help="Append one private spine-performance snapshot.")
    ...
    record.add_argument("--attention-source", choices=ATTENTION_SOURCES, required=True)
    ...
    review = subparsers.add_parser("review", help="Summarise private spine performance by robust medians.")
...
def command_record(args: argparse.Namespace) -> int:
    ...
    print(f"Private spine performance recorded: {path.relative_to(workflow.REPO_ROOT)}")
    print(f"Spine: {record['selected_spine']}; weekday={record['weekday']}; breakout={str(record['is_breakout_outlier']).lower()}.")
