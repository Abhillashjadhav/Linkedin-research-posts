...
def render_dashboard(
    ...
    weekly_card = ""
    weekly = eval_dashboard.get("weekly_plan")
    if isinstance(weekly, Mapping):
        weekly_card = ('<section class="box"><h2>Frozen weekly spine</h2><p>'
                       + _safe(weekly.get("post_date")) + ' · ' + _safe(weekly.get("weekday"))
                       + ' · ' + _safe(weekly.get("style")) + '</p><p>'
                       + _safe(weekly.get("focus")) + '</p>')
        build = weekly.get("build")
        if isinstance(build, Mapping) and isinstance(build.get("video_path"), str):
            video = Path(build["video_path"])
            if video.is_absolute():
                weekly_card += '<p>Video attachment: <a href="' + html.escape(video.as_uri(), quote=True) + '">' + _safe(video.name) + '</a></p>'
        weekly_card += '</section>'
    draft_cards = "".join(
