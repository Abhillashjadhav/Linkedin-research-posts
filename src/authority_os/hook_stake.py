...
#
# It catches rare literary vocabulary. The v2 standard also names one measured
...
def _known_variant(word: str, known: set[str]) -> bool:
    """Recognise conservative inflections without expanding the frozen vocabulary."""
    ...
    for candidate in tuple(variants):
        if candidate.endswith("s") and len(candidate) > 3:
            ...
            variants.add(f"{candidate[:-1]}ed")
        if candidate.endswith("ed") and len(candidate) > 4:
            variants.add(candidate[:-2])
            variants.add(f"{candidate[:-2]}e")
        if candidate.endswith("ing") and len(candidate) > 5:
            variants.add(candidate[:-3])
            variants.add(f"{candidate[:-3]}e")
        if candidate.endswith("able") and len(candidate) > 6:
            variants.add(candidate[:-4])
            variants.add(f"{candidate[:-4]}e")
        variants.add(f"{candidate}ion")
    ...
    for token in re.findall(r"\b[A-Za-z][A-Za-z']{2,}\b", text):
        ...
        for suffix in ("'s", "'"):
            if word.endswith(suffix):
                ...
                break
        if word and not _known_variant(word, known) and word not in found:
            found.append(word)
    return found
